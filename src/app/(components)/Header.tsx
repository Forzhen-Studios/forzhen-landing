...
export default function Header() {
  ...
  const hoverText = useDarkLogo ? 'hover:text-neutral-50' : 'hover:text-neutral-950';
  const circleFill = useDarkLogo ? 'group-hover:fill-neutral-50' : 'group-hover:fill-neutral-950';
  ...
  return (
    <header className="w-full fixed top-0 flex items-center justify-between px-5 py-4 bg-none z-20">
      ...
      >
        <Circle className={`h-2 w-2 inline-block mr-2 duration-100 ease-in-out ${circleFill}`} />
        About Us
      </Link>
    </header>
  );
